...
export class Flecha implements AfterViewInit, OnInit {
  mousePos = null;
  slider: any;
  sliderTxt: any
  playAnim = false;
  ...
  ngAfterViewInit() {
    ...
    //console.log(this.puntosIntermedios);
    this.animate();
  ...
  mueveFlecha(){
    if (this.mousePos && this.isClickDown && !this.pointToMove) {
      for (let punto of this.puntosIntermedios){
        if (Math.round(this.mousePos.x) === Math.round(punto.x) && Math.round(this.mousePos.y)=== Math.round(punto.y)){
          console.log("MOEVMOS")
        }
      }
    }
  ...
  animate() {
    ...
    if (!this.animacionFinalizada) this.mueveFlecha();
