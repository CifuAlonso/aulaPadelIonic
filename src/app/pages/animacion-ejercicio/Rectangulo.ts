...
@Injectable()
export class Rectangulo implements AfterViewInit, OnInit {
  mousePos = null;
  slider: any;
  sliderTxt: any
  playAnim = false;
  ...
  calculaPuntosIntermedios(){
    let xCenter=this.points[0].x+(this.points[3].x-this.points[0].x)/2;
    let yCenter=this.points[0].y+(this.points[3].y-this.points[0].y)/2;
    this.puntoIntermedio=new Point(xCenter,yCenter)
  ...
  drawLine() {
    ...
    this.context.fillRect(this.points[0].x,this.points[0].y,(this.points[3].x-this.points[0].x),(this.points[3].y-this.points[0].y));
